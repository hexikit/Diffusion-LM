...
from infill_util import langevin_fn3, get_score, langevin_fn3_compose, langevin_fn1, langevin_fn4, langevin_fn_tree, langevin_fn_length, langevin_fn_outfit
from spacy.lang.en import English
from torch import nn, optim
from torch.nn import functional as F

from improved_diffusion.rq_vae import RQVAENew, MergedCodebook
from improved_diffusion.outfit_clip import OutfitCLIP
...
def main():
    ...
    clip_model = CLIPModel.from_pretrained(args.clip_model).eval()
    ...
    if args.modality in ['synth', 'pos']:
        ...
        if args.eval_task_ == 'infill':
            ...
            if args.eval_task_ == 'control_attribute':
                model_control = OutfitCLIP(args).cuda()
                # assert encoded_partial_seq[0].size(0)  == 64

                # Tokenize the batch
                prompts = ['Edgy white boy']
                for prompt in prompts:
                    ...
                    with th.no_grad():
                        ...
                        prompt_embs = text_embeds.cpu()

                    prompt_embs = F.normalize(prompt_embs, p=2, dim=1)
                    prompt_embs = prompt_embs.unsqueeze(0)

                    debug_lst = []
                    langevin_fn_selected = partial(langevin_fn_outfit, debug_lst, model_control, model3.cuda(),
                                                    prompt_embs.expand(args.batch_size, -1), 0.1)
                    control_constraints.append((langevin_fn_selected, prompt))
...
def create_argparser():
    ...
    outfit_defaults = dict(
        clip_model='patrickjohncyh/fashion-clip',
        outfit_clip='',
        txt_compress_model='/content/drive/MyDrive/FitFormer/runs/rq_vae/nondisjoint/ae_text_1_c/ep10_checkpoint.pth',
